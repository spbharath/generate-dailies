...
class GenerateDaily:
    ...
    def __init__(self):
        ...
        parser.add_argument(
            "-c",
            "--codec",
            help="Codec name: Possible options are defined in the DAILIES_CONFIG:\n{0}".format(
                "\n\t".join(output_codecs)
            ),
        )
        parser.add_argument(
            "-p",
            "--profile",
            help="Dailies profile: Choose the settings to use for dailies overlays:\n{0}".format(
        ...
        parser.add_argument(
            "-t",
            "--text",
            help='Text elements and contents to add: e.g. \n\t"artist: Jed Smith | comment: this is stupid man|',
        )
        parser.add_argument(
            "-ct",
            "--color_transform",
            help="OCIO Colorspace Conversion preset to use. Specified in the dailies config under ocio_profiles.\n{0}".format(
                " ".join(ocio_profiles.keys())
            ),
        )
        parser.add_argument(
            "--ocio",
            help="OCIO Colorspace Conversion to use. Specified in the dailies config under ocio_profiles.\n{0}".format(
                " ".join(ocio_profiles.keys())
            ),
        )
        parser.add_argument(
        ...
        self.text = {
            "shot_name": "bksm1_BKSM_008_005_MM_V0008",
            "department": "DEPT",
            "version": "VERSION",
            "artist": "ARTISTNAME",
            "framecounter": "",
        }
        self.zero_frame_text = {
            "f_artist": "ARTISTNAME",
            "f_na_na": "NA/NA",
            "f_seq_name": "BKSM1",
            "f_shot_name": "bksm1_BKSM_008_005_MM_V0008",
            "f_asset_name": "SNOW_BACK_106074",
            "f_prev_note_head": "Previous Notes",
            "f_prev_note": "Test Notes for V0007",
            "f_artist_note_head": "Artist Notes",
            "f_artist_note": "Test Notes for V0008",
            "f_frame_slate": "",
            "artist_notes": "Changed something",
        }
        # Modified this as this will input data from thadam
        # for text in texts:
        ...
        if codec not in output_codecs:
            print(
                "Error: invalid codec specified. Possible options are \n\t{0}".format(
                    "\n\t".join(output_codecs)
                )
            )
            self.setup_success = False
        ...
        if self.profile_config.get("text_elements"):
            datetime_format_string = (
                self.profile_config.get("text_elements")
                .get("datetime")
                .get("datetime_format")
            )
            datetime_format_string = (
        ...
        if datetime_format_string:
            self.text["datetime"] = datetime.datetime.now().strftime(
                datetime_format_string
            )
            self.zero_frame_text["f_datetime"] = datetime.datetime.now().strftime(
                datetime_format_string
            )
        else:
            self.text["datetime"] = (
            ...
            self.zero_frame_text["f_datetime"] = (
                datetime.datetime.now().replace(microsecond=0).isoformat()
            )
        ...
        if self.ocioconfig:
            log.debug(
                "Got OCIO config from dailies-config: {0}".format(self.ocioconfig)
            )
        ...
        if commandline_ocio_profile:
            ...
            if commandline_ocio_profile in ocio_profiles.keys():
                self.ociocolorconvert = ocio_profiles.get(commandline_ocio_profile).get(
                    "ociocolorconvert"
                )
            else:
                print(
                    "Error: OCIO color transform {0} does not exist in config. Falling back to default {1}".format(
                        commandline_ocio_profile, ocio_default_transform
                    )
                )
                self.ociocolorconvert = ocio_profiles.get(ocio_default_transform).get(
                    "ociocolorconvert"
                )
            ...
            print(
                "Warning: No default ocio transform specified, and no transform specified on the commandline. No color transform will occur."
            )
    ...
    def process(self):
        ...
        for i, self.frame in enumerate(self.image_sequence, 1):
            ...
            if i == 1:
                buf = self.process_frame(self.frame, first_frame=True)
            else:
                buf = self.process_frame(self.frame)

            if self.zero_frame_text and i == 1:
                self.create_thumbnail(buf)
                buf = self.generate_text(
                    "f_artist",
                    text_elements.get("f_artist"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_na_na",
                    text_elements.get("f_na_na"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_seq_name",
                    text_elements.get("f_seq_name"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_shot_name",
                    text_elements.get("f_shot_name"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_asset_name",
                    text_elements.get("f_asset_name"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_prev_note_head",
                    text_elements.get("f_prev_note_head"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_prev_note",
                    text_elements.get("f_prev_note"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_artist_note_head",
                    text_elements.get("f_artist_note_head"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_artist_note",
                    text_elements.get("f_artist_note"),
                    buf,
                    first_frame=True,
                )
                self.zero_frame_text["f_frame_slate"] = "{0} frames + 1 Slate".format(
                    self.image_sequence.length() - 1
                )
                buf = self.generate_text(
                    "f_frame_slate",
                    text_elements.get("f_frame_slate"),
                    buf,
                    first_frame=True,
                )
                buf = self.generate_text(
                    "f_datetime",
                    text_elements.get("f_datetime"),
                    buf,
                    first_frame=True,
                )

            # Set framecounter in text elements, add framecounter text
            if i != 1 and self.text:
                self.text["framecounter"] = str(self.frame.frame).zfill(
                    text_elements.get("framecounter").get("padding")
                )
                buf = self.generate_text(
                    "framecounter", text_elements.get("framecounter"), buf
                )
    ...
    def create_thumbnail(self, buf):
        input_spec = oiio.ImageSpec()
        input_image = oiio.ImageInput.open(self.first_frame_path)
        input_image.open(self.first_frame_path, input_spec)

        # Get the dimensions of the image
        width = 1920
        height = 1080
        scale_factor = self.profile_config.get("thumbnail").get("scale")
        x_offset = self.profile_config.get("thumbnail").get("offset")[0]
        y_offset = self.profile_config.get("thumbnail").get("offset")[1]

        new_width = int(width * scale_factor)
        new_height = int(height * scale_factor)

        img_buf = oiio.ImageBuf(self.first_frame_path)
        img_buf.read(0, 0, True)

        resized_image = oiio.ImageBuf()
        resized_image.reset(oiio.ImageSpec(new_width, new_height, 3, oiio.FLOAT))
        oiio.ImageBufAlgo.resize(resized_image, img_buf)

        # resized_image.write("D:/Work/generate-dailies/img.jpg")

        # input_image.close()

        x_offset = int(width * x_offset)
        y_offset = int(height * y_offset)
        log.info("w : {0} , h : {1}".format(new_width, new_height))

        try:
            oiio.ImageBufAlgo.paste(buf, x_offset, y_offset, 0, 0, resized_image)
    ...
    def process_frame(self, frame, first_frame=False):
        ...
        #         log.error(
        #             "Cropmask enabled, but no crop specified. Skipping cropmask..."
        #         )
        #     else:
        #         cropmask_height = int(round(self.output_width / cropmask_ar))
        #         cropmask_bar = int((self.output_height - cropmask_height) / 2)
        #         log.debug(
        #             "Cropmask height: \t{0} = {1} / {2} = {3} left".format(
        #                 cropmask_height, self.output_height, cropmask_ar, cropmask_bar
        #             )
        #         )

        #         cropmask_buf = oiio.ImageBuf(
        #             oiio.ImageSpec(
        #                 self.output_width, self.output_height, 4, self.pixel_data_type
        #             )
        #         )
        ...
        #                 self.output_width,
        #                 cropmask_bar,
        #                 self.output_height - cropmask_bar,
        #             ),
        #         )
    ...
    def generate_text(self, text_element_name, text_element, buf, first_frame=False):
        """
        ...
        # Get text to display
        if first_frame:
            text_contents = self.zero_frame_text.get(text_element_name)
        else:
        ...
        if text_contents:
            ...
            if text_roi.width > box_width:
                ...
                for i, word in enumerate(words):
                    ...
                    str_line = " ".join(line)
                    if i < len(words) - 1:
                        text_width = oiio.ImageBufAlgo.text_size(
                            str_line + " " + words[i + 1],
                            fontsize=font_size,
                            fontname=font,
                        ).width
                        ...
                        text_width = oiio.ImageBufAlgo.text_size(
                            str_line, fontsize=font_size, fontname=font
                        ).width
